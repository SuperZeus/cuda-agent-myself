...
static torch::Tensor axpby_forward(torch::Tensor a, torch::Tensor b, double alpha, int config = 0) {
    ...
    TORCH_CHECK(a.sizes() == b.sizes(), "inputs must have identical shapes");
...
static void register_axpby(pybind11::module& module) {
    module.def("axpby_forward", &axpby_forward, py::arg("a"), py::arg("b"), py::arg("alpha"), py::arg("config") = 0);
}
